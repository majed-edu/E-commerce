...
export default function Cart() {
  ...
  if (!items.length) return <div className="container page-shell"><EmptyState title="Your cart is empty" message="Add a few essentials and come back here." action="Shop now" to="/shop" /></div>;
      ...
      <div className="cart-page__grid">
        <div className="cart-box">
          ...
          {items.map((item) => (
            <div className="cart-item" key={item.id}>
              ...
              <button type="button" className="text-btn" onClick={() => removeItem(item.id)}>Remove</button>
            </div>
          ))}
        </div>
        <aside className="summary-box">
          ...
          <div className="promo-box">
            ...
            <div><input id="promo" value={promo} onChange={(event) => setPromo(event.target.value)} placeholder="SAVE10" /><button type="button" onClick={() => setPromo("SAVE10")}>Apply</button></div>
          </div>
          ...
          <Link className="btn btn--primary btn--block" to="/checkout">Proceed to checkout</Link>
        </aside>
      </div>
    </div>
  );
